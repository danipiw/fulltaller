...
// Asegurar que existe la tabla tienda_productos
...
$conn->query("CREATE TABLE IF NOT EXISTS tienda_pedidos (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
$conn->query("CREATE TABLE IF NOT EXISTS tienda_pedido_items (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
if ($orden) {
    ...
    $productos = $conn->query("SELECT * FROM tienda_productos WHERE activo = 1 ORDER BY nombre");
...
<head>
    ...
    <style>
        ...
        .offcanvas-cart { --bs-offcanvas-width: 360px; }
        .cart-item { border-bottom: 1px solid #e9ecef; padding: 10px 0; }
...
<?php
$cart_count = 0;
// contar items del carrito desde cookies/localStorage lo maneja JS
?>
...
<div class="container py-4" style="max-width: 820px;">
    ...
    <!-- TIENDA -->
    <div class="card p-3" id="tienda-section">
        ...
        <div class="row g-3" id="productos-list">
            <?php if ($productos && $productos->num_rows > 0): ?>
                <?php while ($p = $productos->fetch_assoc()): ?>
                <div class="col-6 col-md-4">
                    <div class="producto-card">
                        <?php if ($p['imagen'] && file_exists("tienda_uploads/" . $p['imagen'])): ?>
                        <img src="tienda_uploads/<?php echo htmlspecialchars($p['imagen']); ?>" class="producto-img" alt="<?php echo htmlspecialchars($p['nombre']); ?>">
                        <?php else: ?>
                        <div class="producto-img d-flex align-items-center justify-content-center" style="background:#f8f9fa;">
                            <i class="bi bi-box" style="font-size:2.5rem;color:#adb5bd;"></i>
                        </div>
                        <?php endif; ?>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-2">
                            ...
                            <button class="btn btn-sm btn-outline-primary add-to-cart" data-id="<?php echo $p['id']; ?>" data-nombre="<?php echo htmlspecialchars($p['nombre']); ?>" data-precio="<?php echo $p['precio']; ?>">
            ...
            <div class="col-12 text-center py-4 text-muted">
                ...
                <p class="mt-2">Próximamente disponibles</p>
            </div>
            <?php endif; ?>
        </div>
    </div>
...
<!-- Carrito flotante -->
<div class="badge-carrito" id="cart-badge" onclick="openCart()" style="display:none;">
    <span id="cart-count">0</span>
</div>

<!-- Offcanvas Carrito -->
<div class="offcanvas offcanvas-end offcanvas-cart" tabindex="-1" id="cart-offcanvas">
    ...
    <div class="offcanvas-body">
        ...
        <button class="btn btn-primary w-100 mt-3" onclick="checkout()" id="btn-checkout">
...
function updateCartUI() {
    ...
    if (cart.length === 0) {
        container.innerHTML = '<div class="text-center text-muted py-4"><i class="bi bi-cart" style="font-size:2rem;"></i><p class="mt-2">Carrito vacío</p></div>';
        return;
    }
    container.innerHTML = cart.map((item, i) => `
        <div class="cart-item d-flex justify-content-between align-items-center">
            <div>
                ...
                <div style="font-size:0.8rem;color:#6c757d;">$${item.precio.toFixed(2)} x ${item.cantidad}</div>
            </div>
...
function checkout() {
    ...
    const wa = prompt('WhatsApp del taller (código de país + número, ej: 5491123456789):', '<?php echo htmlspecialchars($config_s['taller_telefono'] ?? ''); ?>');
...
document.querySelectorAll('.add-to-cart').forEach(btn => {
    btn.addEventListener('click', function() {
        addToCart(this.dataset.id, this.dataset.nombre, this.dataset.precio);
    });
});
...
<head>
    ...
    <style>
        ...
        .form-control:focus { border-color: #0f3460; box-shadow: 0 0 0 3px rgba(15,52,96,0.15); }
        .btn-primary {
        ...
        .store-teaser { text-align: center; margin-top: 20px; padding: 16px; background: rgba(255,255,255,0.05); border-radius: 12px; color: rgba(255,255,255,0.8); font-size: 0.85rem; }
        .store-teaser i { font-size: 1.5rem; color: #0dcaf0; }
        @media (max-width: 480px) {
            .buscar-card { padding: 28px 20px; border-radius: 16px; }
        }
    </style>
</head>
<body>
...
<div class="container" style="max-width:460px;padding-top:60px;">
    ...
    <div class="store-teaser">
        <i class="bi bi-bag"></i>
        <p class="mt-1 mb-0">¿Necesitás accesorios? Consultá después de ver tu orden.</p>
    </div>
